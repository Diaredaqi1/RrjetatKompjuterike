...
#MSH1 -> Gjen syprinen e siperfaqes se drejtekendeshit duke i marr brinjet e tij si argumente
...
class ClientThread(threading.Thread):
    def __init__(self,address,clientS):
        ...
        self.csocket = clientS
        print ("Lidhja e klientit u be ne: ", address)
        print('\n')

    def run(self):

         while True:
            kerkesa = self.csocket.recv(128)
            ...
            self.csocket.send(bytes(pergjigjja,'UTF-8'))
...
try:
    while True:
        ...
        print('Lidhja klient server u realizua')
        newthread = ClientThread(address, clientS)
        newthread.start()
        threads.append(newthread)
    for i in threads:
        i.join()
except Exception:
    print('Ndodhi nje problem')
